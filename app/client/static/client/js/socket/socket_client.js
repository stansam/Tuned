class SocketClient {
    ...
    initializeSounds() {
        return {
            message: new Audio('/static/client/sounds/message.mp3'),
            notification: new Audio('/static/client/sounds/notification.mp3'),
            alert: new Audio('/static/client/sounds/alert.mp3'),
            success: new Audio('/static/client/sounds/success.mp3')
        };
    }
    ...
    setupEventListeners() {
        ...
        this.socket.on('all_notifications_loaded', (data) => {
            this.handleAllNotificationsLoaded(data);
        });
    ...
    handleAllNotificationsLoaded(data) {
        // This will be called when all notifications are loaded for modal
        if (window.notificationManager) {
            window.notificationManager.handleNotificationsLoaded(data);
        }
    }
    ...
    handleNewNotification(data) {
        ...
        // Add to modal if it's Open
        if (notificationManager && document.getElementById('notif-modal-overlay').classList.contains('show')) {
            notificationManager.allNotifications.unshift(notification);
            notificationManager.filterNotifications();
            notificationManager.renderModalNotifications();
        }
    }
    ...
    handleNotificationMarkedRead(notificationId) {
        ...
        const modalElement = document.querySelector(`#notif-modal-list [data-notification-id="${notificationId}"]`);
        if (modalElement) {
            modalElement.classList.add('read');
            modalElement.classList.remove('unread');
            
            // Remove unread indicator and mark as read button
            const modalUnreadIndicator = modalElement.querySelector('.unread-indicator');
            if (modalUnreadIndicator) {
                modalUnreadIndicator.remove();
            }
            
            const markReadBtn = modalElement.querySelector('button');
            if (markReadBtn && markReadBtn.textContent.includes('Mark as Read')) {
                markReadBtn.remove();
            }
        }
        
        // Clear read notifications from panel after a delay
        setTimeout(() => {
    ...
    addNotificationToPanel(notification) {
        const notificationsList = document.getElementById('notifications-list');
        if (!notificationsList || notification.is_read) return;
        
        const existingNotification = document.querySelector(`#notifications-list [data-notification-id="${notification.id}"]`);
    ...
    updateUnreadCountsUI() {
        ...
        if (notificationBadge) {
            if (this.unreadCounts.notifications > 0) {
                notificationBadge.textContent = this.unreadCounts.notifications > 99 ? '99+' : this.unreadCounts.notifications;
        ...
        const fabMessagesBadge = document.getElementById('fabChatNotificationBadge');
        if (messagesBadge) {
            if (this.unreadCounts.messages > 0) {
                messagesBadge.textContent = this.unreadCounts.messages;
                fabChatNotificationBadge.textContent = this.unreadCounts.messages;
                messagesBadge.style.display = 'block';
                fabMessagesBadge.style.display = 'block';
            } else {
                messagesBadge.style.display = 'none';
                fabMessagesBadge.style.display = 'none';
            }
        }
